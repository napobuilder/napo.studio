...
let globalStreamDest    = null;   
let globalMediaRecorder = null;   
let globalRecChunks     = [];     
let globalTrackGains    = {};     // Nodos de volumen individuales por pista
...
export default function App() {
  ...
  const handlePlaySnippet = (workId, url) => {
    ...
    if (!isPaused && hasEntered && !hasSaved) {
      if (globalAudioCtx) globalAudioCtx.suspend(); // Pausa perfecta de stems
  ...
  const handleEnter = () => {
    ...
    // Configurar grabadora de forma segura
    globalRecChunks = [];
    const mimeType = [
    ...
    try {
      if (window.MediaRecorder) {
        globalMediaRecorder = new MediaRecorder(globalStreamDest.stream, mimeType ? { mimeType } : {});
        globalMediaRecorder.ondataavailable = (e) => {
          if (e.data && e.data.size > 0) globalRecChunks.push(e.data);
        };
        globalMediaRecorder.start(500); 
        setIsRecording(true);
      }
    } catch(err) {
      console.error("No recorder:", err);
      setIsRecording(true); // Permitimos que la UI siga aunque no grabe
    }

    // Resetear las ganancias (Solo Ether activo al inicio con setValueAtTime por seguridad iOS)
    ['ether', 'bass', 'arp', 'drums'].forEach(id => {
      if (globalTrackGains[id]) {
        globalTrackGains[id].gain.setValueAtTime(id === 'ether' ? 1 : 0, globalAudioCtx.currentTime);
      }
    });
    ...
    ['ether', 'bass', 'arp', 'drums'].forEach(id => {
      if (globalAudioBuffers[id]) {
        ...
        source.buffer = globalAudioBuffers[id];
        source.loop = true; // Loop infinito nativo, sin gaps
  ...
  const toggleTrack = (trackId) => {
    setTrackStates(prev => {
      ...
      if (globalTrackGains[trackId] && globalAudioCtx) {
        // Hacemos una transición suavizada (ramp) de 50ms para que no haya "clics" de audio
  ...
  const handleStopAndSave = () => {
    ...
    // Apagar las fuentes de audio
    ['ether', 'bass', 'arp', 'drums'].forEach(id => {
      if (globalBufferSources[id]) {
        try { globalBufferSources[id].stop(); } catch(e){}
      }
    });
  ...
  const handleReset = () => {
    ...
    if (globalMediaRecorder && globalMediaRecorder.state !== 'inactive') {
      globalMediaRecorder.onstop = null; 
      globalMediaRecorder.stop();
    }
    globalMediaRecorder = null;
    globalRecChunks = [];
  ...
  return (
    <div className={`bg-[#050505] text-[#9ca3af] font-mono selection:bg-[#9D4EDD] selection:text-white min-h-screen relative scroll-smooth ${isPaused ? 'paused-state' : ''}`}>
      ...
      `}</style>
      {/* Solo dejamos la etiqueta audio para los snippets que no requieren sincro */}
      <audio 
        ref={snippetAudioRef} 
